...
import com.changan.carbond.common.utils.ListUtils;
...
public abstract class BaseServiceImpl<T extends BaseModelObject> implements IBaseService<T> {
    ...
    private static int BATCH_PER_SQL_NUMBER = 100;
    ...
    public Msg<List<T>> batchInsert(List<T> records) {
        ...
        int insertCount = 0;
        List<List<T>> lists = ListUtils.splitList(records, BATCH_PER_SQL_NUMBER);
    ...
    public Msg<T> selectOneByExample(T record, boolean distinct) {
        Msg<T> msg = new Msg<>();
        List<T> result = getRepositoryDao().selectByExample(record, true);
